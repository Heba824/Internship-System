...
import { 
  FaBars, FaTimes, FaFileAlt, FaHome, 
  FaSignOutAlt, FaDownload, FaTimesCircle,
  FaCheck, FaFlag, FaHourglassHalf, FaClock, FaSync,
  FaStar, FaBuilding, FaBook, FaChartBar, FaUser, FaCalendarAlt, FaComment, FaPaperclip
} from 'react-icons/fa';
...
const Faculty = () => {
    ...
    const initialReports = [
      {
        ...
        performanceEvaluation: {
          ...
          overallAssessment: "The student demonstrated exceptional skills in programming and showed great potential for growth."
        }
      },
      {
        id: 2,
        ...
        performanceEvaluation: {
          ...
          teamwork: "Excellent",
          communication: "Good",
          overallAssessment: "The student showed strong technical abilities but needs to work on documentation skills."
        }
      },
      {
        id: 3,
        ...
        endDate: "2023-09-15",
        reportContent: "Worked on optimizing data structures for the company's core product...",
        learningOutcomes: "Gained practical experience with large-scale data processing."
      }
    ];
  ...
  const calculateInternshipStats = useCallback(() => {
    ...
    internshipReports.forEach(report => {
      if (report.company) {
        const rating = 
          report.status === 'Accepted' ? 4.5 :
          report.status === 'Flagged' ? 3.0 :
          report.status === 'Pending' ? 2.5 : 1.0;
  ...
  const handleDownloadReport = async () => {
    try {
      ...
      if (selectedReport.status) {
        const statusColor = 
          selectedReport.status === 'Accepted' ? [76, 175, 80] :
          selectedReport.status === 'Pending' ? [255, 165, 0] :
          selectedReport.status === 'Flagged' ? [255, 193, 7] : [244, 67, 53];
      ...
      if (selectedReport.internshipDetails) {
        ...
        addField('Hours Completed', selectedReport.internshipDetails.hoursCompleted);
      ...
      const reportClarification = clarifications[selectedReport.id] || selectedReport.clarification;
      if (reportClarification) {
        doc.setFontSize(14);
        doc.setTextColor(244, 67, 53);
        doc.text(
          `${selectedReport.status === 'Flagged' ? 'Flagged' : 'Rejected'} Report Clarification`, 
          14, 
          yPosition
        );
        yPosition += 10;
        ...
        doc.setTextColor(0, 0, 0);
        const clarificationLines = doc.splitTextToSize(reportClarification.text, 180);
      ...
      doc.save(`${selectedReport.title.replace(/[^a-z0-9]/gi, '_') || 'report'}.pdf`);
  ...
  const generateReport = (type) => {
    ...
    setTimeout(() => {
      ...
      reportContent += `Average Review Time: ${realTimeStats.reports?.averageReviewTime || "N/A"}\n\n`;
      
      if (realTimeStats.internshipStats) {
        reportContent += `=== INTERNSHIP INSIGHTS ===\n`;
        
        reportContent += `Most Frequent Courses:\n`;
        realTimeStats.internshipStats.frequentCourses.forEach((course, index) => {
          reportContent += `${index + 1}. ${course.course}: ${course.count} reports\n`;
        });
        reportContent += '\n';
        
        reportContent += `Top Rated Companies:\n`;
        realTimeStats.internshipStats.topRatedCompanies.forEach((company, index) => {
          reportContent += `${index + 1}. ${company.company}: ${company.averageRating} stars (${company.count} reports)\n`;
        });
        reportContent += '\n';
        
        reportContent += `Most Active Companies:\n`;
        realTimeStats.internshipStats.topCompaniesByCount.forEach((company, index) => {
          reportContent += `${index + 1}. ${company.company}: ${company.count} internships\n`;
        });
      }
  ...
  const updateReportStatus = (status) => {
    ...
    if ((status === 'Flagged' || status === 'Rejected') && !clarificationText.trim()) {
      ...
      <div style={styles.pdfPreview}>
        ...
        <div style={styles.pdfContent}>
          <div style={styles.pdfSection}>
            <h3>
              {selectedReport?.type === 'research' ? 'Internship' : 'Evaluation'} 
              Report Information
            </h3>
            <p><strong>Course:</strong> {selectedReport?.course}</p>
            <p><strong>Major:</strong> {selectedReport?.major}</p>
            <p><strong>Submitted:</strong> {selectedReport?.submissionDate}</p>
            <p><strong>Status:</strong> 
              <span style={{
                color: selectedReport?.status === 'Accepted' ? '#4CAF50' :
                      selectedReport?.status === 'Pending' ? '#FFA500' :
                      selectedReport?.status === 'Flagged' ? '#FFC107' : '#f44336',
                fontWeight: 'bold',
                marginLeft: '5px'
              }}>
          ...
          {selectedReport?.student && (
            <div style={styles.pdfSection}>
              <h3><FaUser style={{ marginRight: '10px' }} />Student Details</h3>
              <div style={styles.detailGrid}>
                <div>
                  <p><strong>Name:</strong> {selectedReport.student.name}</p>
                  <p><strong>Student ID:</strong> {selectedReport.student.id}</p>
                </div>
                <div>
                  <p><strong>Email:</strong> {selectedReport.student.email}</p>
                  <p><strong>Program:</strong> {selectedReport.student.program}</p>
                </div>
              </div>
            </div>
          )}
          ...
          {selectedReport?.internshipDetails && (
            <div style={styles.pdfSection}>
              <h3><FaBuilding style={{ marginRight: '10px' }} />Internship Details</h3>
              <div style={styles.detailGrid}>
                <div>
                  <p><strong>Company:</strong> {selectedReport.internshipDetails.companyName}</p>
                  <p><strong>Position:</strong> {selectedReport.internshipDetails.position}</p>
                </div>
                <div>
                  <p><strong>Supervisor:</strong> {selectedReport.internshipDetails.supervisor}</p>
                  <p><strong>Hours Completed:</strong> {selectedReport.internshipDetails.hoursCompleted}</p>
                </div>
              </div>
              <div style={styles.detailGrid}>
                <div>
                  <p><strong><FaCalendarAlt style={{ marginRight: '5px' }} />Start Date:</strong> {selectedReport.internshipDetails.startDate}</p>
                </div>
                <div>
                  <p><strong><FaCalendarAlt style={{ marginRight: '5px' }} />End Date:</strong> {selectedReport.internshipDetails.endDate}</p>
                </div>
              </div>
            </div>
          )}
          ...
          {selectedReport?.evaluationDetails && (
            <div style={styles.pdfSection}>
              ...
              <div style={styles.detailGrid}>
                <div>
                  <p><strong>Grade:</strong> {selectedReport.evaluationDetails.grade}</p>
                  <p><strong>Credits Earned:</strong> {selectedReport.evaluationDetails.creditsEarned}</p>
                </div>
                <div>
                  <p><strong>Evaluator:</strong> {selectedReport.evaluationDetails.evaluator}</p>
                  <p><strong>Evaluation Date:</strong> {selectedReport.evaluationDetails.evaluationDate}</p>
                </div>
              </div>
            </div>
          )}
          ...
          {selectedReport?.performanceEvaluation && (
            <div style={styles.pdfSection}>
              ...
              <div style={styles.detailGrid}>
                <div>
                  <p><strong>Technical Skills:</strong> {selectedReport.performanceEvaluation.technicalSkills}</p>
                  <p><strong>Problem Solving:</strong> {selectedReport.performanceEvaluation.problemSolving}</p>
                </div>
                <div>
                  <p><strong>Teamwork:</strong> {selectedReport.performanceEvaluation.teamwork}</p>
                  <p><strong>Communication:</strong> {selectedReport.performanceEvaluation.communication}</p>
                </div>
              </div>
          ...
          {selectedReport?.type === 'research' && (
            <>
              <div style={styles.pdfSection}>
                ...
                <p>{selectedReport?.reportContent || "No content available."}</p>
              </div>
              
              <div style={styles.pdfSection}>
                <h3>Learning Outcomes</h3>
                <p>{selectedReport?.learningOutcomes || "No learning outcomes provided."}</p>
              </div>
            </>
          )}
          ...
          {reportClarification && (
            <div style={styles.pdfSection}>
              ...
              <p><strong>Reason:</strong> {reportClarification.text}</p>
              <p><strong>Date:</strong> {new Date(reportClarification.date).toLocaleString()}</p>
  ...
  const renderReportsTab = () => {
    ...
    const filteredReports = reports.filter(report => {
      ...
      const majorMatch = selectedMajor === 'All' || report.major === selectedMajor;
    ...
    return (
      <div style={styles.pageContainer}>
        <h1 style={styles.pageTitle}><FaFileAlt style={styles.titleIcon} /> Reports</h1>
          ...
          <div style={{ 
            ...
            <button
              ...
              style={{
                ...
                backgroundColor: reportCategory === 'all' ? '#05445E' : '#f0f2f5',
            ...
            <button
              ...
              style={{
                ...styles.categoryButton,
                backgroundColor: reportCategory === 'evaluation' ? '#05445E' : '#f0f2f5',
            ...
            <button
              ...
              style={{
                ...styles.categoryButton,
                backgroundColor: reportCategory === 'internship' ? '#05445E' : '#f0f2f5',
        ...
        <div style={styles.filterContainer}>
          ...
          <select
            ...
            onChange={(e) => {
              ...
              setReportCategory(
                e.target.value === 'grades' ? 'evaluation' :
                e.target.value === 'research' ? 'internship' : 'all'
              );
            }}
                ...
                <li 
                  ...
                  style={{
                    ...
                    borderLeft: `4px solid ${
                      ...
                      report.status === 'Pending' ? '#FFA500' : 
                      report.status === 'Flagged' ? '#FFC107' : '#f44336'
                    }`,
                    ':hover': {
                ...
                >
                  <h3 style={{ marginTop: 0, color: '#05445E' }}>
                    ...
                    <span style={{ 
                      fontSize: '14px', 
                      marginLeft: '10px',
                      backgroundColor: report.type === 'research' ? '#05445E' : '#189AB4',
                      color: 'white',
                      padding: '3px 8px',
                      borderRadius: '4px'
                    }}>
                  ...
                  {report.course && <p><strong>Course:</strong> {report.course}</p>}
                  {report.major && <p><strong>Major:</strong> {report.major}</p>}
                  <p><strong>Submitted:</strong> {report.submissionDate}</p>
                  <p style={{
                    color: report.status === 'Accepted' ? '#4CAF50' : 
                          report.status === 'Pending' ? '#FFA500' : 
                          report.status === 'Flagged' ? '#FFC107' : '#f44336',
                    fontWeight: 'bold'
                  }}>
      ...
      <div style={styles.dashboardContent}>
        <div style={styles.welcomeBox}>
          ...
          <p style={styles.welcomeText}>
            Welcome to your faculty dashboard. Here you can view reports and statistics.
          </p>
        </div>
        ...
        <div style={{ marginBottom: '40px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              {lastUpdate && <small style={{ color: '#666' }}>Last updated: {lastUpdate}</small>}
              <button 
                onClick={updateStats}
                style={{
              ...
              <div style={{ marginTop: '40px' }}>
                ...
                <div style={{ 
                  ...
                  <div style={{ 
                    ...
                    <h3 style={{ marginTop: 0, color: '#05445E', display: 'flex', alignItems: 'center', gap: '10px' }}>
                      <FaBook color="#189AB4" /> Most Frequent Courses
                    </h3>
                    <ul style={{ listStyle: 'none', padding: 0 }}>
                      {realTimeStats.internshipStats?.frequentCourses.map((item, index) => (
                        <li key={index} style={{ padding: '8px 0', borderBottom: '1px solid #eee', display: 'flex', justifyContent: 'space-between' }}>
                          <span>{item.course}</span>
                          <span style={{ fontWeight: 'bold' }}>{item.count} reports</span>
                        </li>
                      ))}
                    </ul>
                  </div>
                  ...
                  <div style={{ 
                    ...
                    <ul style={{ listStyle: 'none', padding: 0 }}>
                      {realTimeStats.internshipStats?.topRatedCompanies.map((item, index) => (
                        <li key={index} style={{ padding: '8px 0', borderBottom: '1px solid #eee', display: 'flex', justifyContent: 'space-between' }}>
                          <span>{item.company}</span>
                          <span style={{ fontWeight: 'bold' }}>{item.averageRating} ★ ({item.count} reports)</span>
                        </li>
                      ))}
                    </ul>
                  </div>
                  ...
                  <div style={{ 
                    ...
                    <ul style={{ listStyle: 'none', padding: 0 }}>
                      {realTimeStats.internshipStats?.topCompaniesByCount.map((item, index) => (
                        <li key={index} style={{ padding: '8px 0', borderBottom: '1px solid #eee', display: 'flex', justifyContent: 'space-between' }}>
                          <span>{item.company}</span>
                          <span style={{ fontWeight: 'bold' }}>{item.count} internships</span>
                        </li>
                      ))}
                    </ul>
                  </div>
