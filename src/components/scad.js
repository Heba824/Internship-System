...
import { 
  FaBars, FaTimes, FaBuilding, FaUserGraduate, FaChalkboardTeacher, 
  FaBriefcase, FaFileAlt, FaHome, FaSignOutAlt, FaDownload, FaTimesCircle,
  FaCheck, FaFlag, FaHourglassHalf, FaClock, FaSync, FaCalendar,FaChartBar,
  FaCheckCircle, FaTimes as FaReject
} from 'react-icons/fa';
...
function Scad() {
  ...
  const [selectedInternshipIndustry, setSelectedInternshipIndustry] = useState('All');
  const [selectedInternshipDuration, setSelectedInternshipDuration] = useState('All');
  const [selectedInternshipPayment, setSelectedInternshipPayment] = useState('All');
  ...
  const [internships, setInternships] = useState([
    { 
      ...
      requirements: 'Knowledge of JavaScript, HTML/CSS. React experience preferred.',
      location: 'Remote',
      paid: true,
      stipend: '$2000/month'
    },
    ...
    { 
      ...
      endDate: '2023-12-31',
      description: 'Learn and implement security best practices in real projects.',
  ...
  const [companies, setCompanies] = useState([
    { id: 1, name: 'TechNova Solutions', industry: 'Software', status: 'Pending', description: 'Leading software solutions for businesses.', address: '123 Tech Lane' },
    { id: 2, name: 'GreenCore Industries', industry: 'Environmental Tech', status: 'Pending', description: 'Innovative environmental technology solutions.', address: '456 Green Street' },
    { id: 3, name: 'EduBridge Academy', industry: 'Education', status: 'Pending', description: 'Education platform for bridging learning gaps.', address: '789 Bridge Blvd' },
    { id: 4, name: 'CodeHive', industry: 'Software', status: 'Pending', description: 'Coding community for aspiring developers.', address: '101 Code Ave' },
    { id: 5, name: 'BioGen Labs', industry: 'Biotech', status: 'Pending', description: 'Biotech research for health innovations.', address: '202 Bio Rd' }
  ]);
  ...
  const students = [
    ...
    { 
      ...
      gpa: '3.7',
      year: 'Sophomore',
      skills: ['Video Editing', '3D Animation', 'Graphic Design', 'VR Development'],
...
const submittedReports = [
  ...
  const calculateCompanyStats = useCallback(() => {
    ...
    const ratings = {};
    students.forEach(student => {
      if (student.internshipStatus === 'Accepted' && student.internshipCompany) {
    ...
    while (topByCount.length < 3 && acceptedCompanies[topByCount.length]) {
      topByCount.push({ name: acceptedCompanies[topByCount.length].name, count: 1 });
    }
    while (topRated.length < 3 && acceptedCompanies[topRated.length]) {
      topRated.push({ name: acceptedCompanies[topRated.length].name, rating: 4.0 });
    }
  ...
  const calculateCycleStats = useCallback(() => {
    ...
    const studentsInCycle = students.filter(student => 
  ...
  }, [calculateReportStats, calculateCourseStats, calculateCompanyStats, calculateCycleStats]);
  ...
  const handleInternshipCycleSubmit = () => {
    ...
    if (new Date(internshipCycle.startDate) >= new Date(internshipCycle.endDate)) {
  ...
  const handleDownloadReport = () => {
    ...
    doc.text(selectedReport?.title || 'Internship Report', 105, 20, { align: 'center' });
    ...
    doc.text(`Company: ${selectedReport?.company || 'TechNova Solutions'}`, 15, 90);
    doc.text(`Supervisor: ${selectedReport?.supervisor || 'Jane Smith'}`, 15, 98);
    doc.text(`Period: ${selectedReport?.startDate || '2023-06-01'} to ${selectedReport?.endDate || '2023-08-31'}`, 15, 106);
    ...
    if (selectedReport?.type === 'evaluation') {
      ...
      doc.text(`Technical Skills: ${selectedReport?.technicalSkills || '4.0'}/5`, 105, 50);
      doc.text(`Problem Solving: ${selectedReport?.problemSolving || '4.0'}/5`, 105, 58);
      doc.text(`Teamwork: ${selectedReport?.teamwork || '5.0'}/5`, 105, 66);
      doc.text(`Communication: ${selectedReport?.communication || '4.5'}/5`, 105, 74);
      doc.text(`Professionalism: ${selectedReport?.professionalism || '5.0'}/5`, 105, 82);
      ...
      const overallRating = (
        (parseFloat(selectedReport?.technicalSkills || 4) +
         ...
         parseFloat(selectedReport?.professionalism || 5)) / 5
      ).toFixed(1);
    ...
    doc.save(`${selectedReport?.studentName || 'Student'}_${selectedReport?.title || 'Report'}.pdf`);
  };
  ...
  const renderCompaniesTab = () => {
    ...
    return (
      <div style={styles.pageContainer}>
        <h1 style={styles.pageTitle}><FaBuilding style={styles.titleIcon} /> Company Management</h1>
                  ...
                  {filteredCompanies.map(company => (
                    <tr 
                      ...
                      style={{
                        ...
                        backgroundColor: 
                          company.status === 'Accepted' ? '#f0fff0' : 
                          company.status === 'Rejected' ? '#fff0f0' : '#fff'
                      }}
                    >
                      ...
                      <td style={styles.tableCell}>
                        <span style={{ 
                          color: 
                            company.status === 'Accepted' ? '#4CAF50' : 
                        ...
                        <div style={styles.actionButtons}>
                          ...
                          {company.status === 'Pending' && (
                            <>
                              ...
                              >
                                <FaReject style={{ marginRight: '5px' }} /> Reject
                              </button>
                            </>
                          )}
                          {company.status !== 'Pending' && (
                            <button
                              onClick={() => {
                                if (company.status === 'Accepted') {
                                  ...
                                  if (window.confirm('Are you sure you want to accept this company?')) {
                                    handleAcceptCompany(company.id);
                                  }
                                }
                              }}
                              style={company.status === 'Accepted' ? styles.rejectButton : styles.acceptButton}
                            >
                              {company.status === 'Accepted' ? (
                                <><FaReject style={{ marginRight: '5px' }} /> Reject</>
                              ) : (
        ...
        {isModalOpen && selectedCompany && (
          <div style={styles.modalOverlay}>
            <div style={styles.modalContent}>
              <h2 style={{ marginTop: 0, color: '#05445E' }}>{selectedCompany.name}</h2>
              <div style={styles.modalSection}>
                <h3 style={styles.modalSubtitle}>Company Details</h3>
                <p><strong>Industry:</strong> {selectedCompany.industry}</p>
                <p><strong>Status:</strong> 
                  <span style={{ 
                    color: selectedCompany.status === 'Accepted' ? '#4CAF50' : 
                          selectedCompany.status === 'Rejected' ? '#f44336' : 
                          '#FFA500',
                    fontWeight: 'bold',
                    marginLeft: '5px'
                  }}>
              ...
              <div style={styles.modalActionButtons}>
                {selectedCompany.status === 'Pending' && (
                  <>
                    ...
                    >
                      <FaCheckCircle style={{ marginRight: '8px' }} /> Accept Company
                    </button>
                    <button 
                ...
                {selectedCompany.status !== 'Pending' && (
                  <button
                    ...
                    style={selectedCompany.status === 'Accepted' ? styles.modalRejectButton : styles.modalAcceptButton}
                  >
      ...
      <div style={styles.pdfPreview}>
        ...
        <div style={styles.pdfContent}>
          <div style={styles.pdfSection}>
            ...
            <p><strong>Name:</strong> {selectedReport?.studentName || 'John Doe'}</p>
            <p><strong>Major:</strong> {selectedReport?.major || 'Computer Science'}</p>
            <p><strong>Company:</strong> {selectedReport?.company || 'TechNova Solutions'}</p>
            <p><strong>Supervisor:</strong> {selectedReport?.supervisor || 'Jane Smith'}</p>
            <p><strong>Internship Period:</strong> {selectedReport?.startDate} to {selectedReport?.endDate}</p>
            
            {isEvaluation && (
              <>
                <p><strong>Evaluator:</strong> {selectedReport?.evaluator || "Dr. Sarah Johnson"}</p>
                <p><strong>Evaluation Date:</strong> {selectedReport?.evaluationDate || new Date().toLocaleDateString()}</p>
              </>
            )}
          </div>
          ...
          {isEvaluation ? (
            <>
              <div style={styles.pdfSection}>
                ...
                <p>This report evaluates the student's performance during their internship period at {selectedReport?.company}. 
                The evaluation is based on technical skills, professional behavior, and overall contribution to the host organization.</p>
              </div>
              ...
              <div style={styles.pdfSection}>
                ...
                <div style={styles.assessmentGrid}>
                  <div style={styles.assessmentItem}>
                    <strong>Technical Skills:</strong> {selectedReport?.technicalSkills}/5
                  </div>
                  <div style={styles.assessmentItem}>
                    <strong>Problem Solving:</strong> {selectedReport?.problemSolving}/5
                  </div>
                  <div style={styles.assessmentItem}>
                    <strong>Teamwork:</strong> {selectedReport?.teamwork}/5
                  </div>
                  <div style={styles.assessmentItem}>
                    <strong>Communication:</strong> {selectedReport?.communication}/5
                  </div>
                  <div style={styles.assessmentItem}>
                    <strong>Professionalism:</strong> {selectedReport?.professionalism}/5
                  </div>
                </div>
              </div>
              ...
              <div style={styles.pdfSection}>
                ...
                <p>{selectedReport?.supervisorComments || "The student demonstrated strong technical abilities and adapted well to our team environment. They showed consistent improvement throughout the internship period."}</p>
              </div>
            </>
          ) : (
            <>
              <div style={styles.pdfSection}>
                ...
                <p>This report details my internship experience at {selectedReport?.company} where I worked on various projects. I gained valuable experience and skills during this period.</p>
              </div>
                ...
                {reportClarifications.length > 0 ? (
                  reportClarifications.map((clarification, index) => (
                    <div key={index} style={styles.clarificationItem}>
                      <p><strong>Student:</strong> {clarification.text}</p>
                      <p><small>{new Date(clarification.date).toLocaleString()}</small></p>
                      {clarification.response && (
                        <div style={styles.adminResponse}>
                          <p><strong>Admin Response:</strong> {clarification.response}</p>
                          <p><small>{new Date(clarification.responseDate).toLocaleString()}</small></p>
                        </div>
                      )}
                    </div>
                  ))
                  ...
                  <div style={styles.adminResponseForm}>
                    ...
                    <button 
                      onClick={() => {
                        ...
                        const lastClarification = updatedClarifications[updatedClarifications.length - 1];
                        lastClarification.response = adminResponse;
                        lastClarification.responseDate = new Date().toISOString();
          ...
          <div style={styles.pdfSection}>
            ...
            <p>
              {isEvaluation 
                ? selectedReport?.overallAssessment || "The student demonstrated excellent performance throughout the internship, showing strong technical abilities and professional demeanor. They exceeded expectations in all evaluation criteria."
            ...
            {isEvaluation && (
              <p style={{ marginTop: '10px', fontWeight: 'bold' }}>
                Overall Rating: {selectedReport?.overallRating || 
                  ((selectedReport?.technicalSkills + selectedReport?.problemSolving + selectedReport?.teamwork + 
                    selectedReport?.communication + selectedReport?.professionalism) / 5).toFixed(1)}/5
              </p>
            )}
          </div>
        </div>
  ...
  const generateReport = (type) => {
    ...
    setTimeout(() => {
      ...
      const reportStats = [
        ...
        { label: 'Average Review Time', value: realTimeStats.reports?.averageReviewTime || 'N/A' }
      ];
      ...
      if (realTimeStats.courses?.distribution) {
        Object.entries(realTimeStats.courses.distribution).forEach(([course, count], index) => {
          if (index < 5) { // Limit to top 5 courses
      ...
      if (realTimeStats.companies?.topRated?.length) {
        realTimeStats.companies.topRated.forEach((company, index) => {
          doc.text(`${company.name}: ${company.rating}/5`, 30, 235 + (index * 10));
        });
      } else {
  ...
  const renderPageContent = () => {
    switch(activeTab) {
              ...
              {filteredCompanies.length > 0 ? (
                filteredCompanies.map((company) => (
                  ...
                  >
                    ...
                    Status: <span style={{ 
                      color: company.status === 'Accepted' ? '#4CAF50' : 
                            company.status === 'Rejected' ? '#f44336' : 
                            '#189AB4',
                      fontWeight: 'bold'
                    }}>
      ...
      case 'internships':
        const companyOptions = ['All', ...new Set(internships.map(i => i.company))];
        const durationOptions = ['All', ...new Set(internships.map(i => i.duration))];
        const industryOptions = ['All', ...new Set(companies.map(c => c.industry))];
        ...
        const filteredInternships = internships.filter(internship => {
          ...
          const companyObj = companies.find(c => c.name === internship.company);
        ...
        return (
          <div style={styles.pageContainer}>
            <h1 style={styles.pageTitle}><FaBriefcase style={styles.titleIcon} /> Available Internships</h1>
                  ...
                  {filteredInternships.map(internship => (
                    <li 
                      ...
                      style={{
                        ...
                        border: selectedInternship?.id === internship.id ? '2px solid #189AB4' : 'none',
                        ':hover': {
                    ...
                    >
                      <h3 style={{ marginTop: 0, color: '#05445E' }}>{internship.title}</h3>
                      <p><strong>Company:</strong> {internship.company}</p>
                      <p><strong>Industry:</strong> {
                        companies.find(c => c.name === internship.company)?.industry || 'Other'
                      }</p>
                      <p><strong>Duration:</strong> {internship.duration}</p>
                      <p><strong>Compensation:</strong> {internship.paid ? 
                        `${internship.stipend} (Paid)` : 'Unpaid (Academic Credit)'}</p>
                    </li>
                  ))}
                </ul>
              ) : (
            ...
            {isInternshipModalOpen && selectedInternship && (
              <div style={styles.modalOverlay}>
                <div style={styles.modalContent}>
                  <h2 style={{ marginTop: 0, color: '#05445E' }}>{selectedInternship.title}</h2>
                  <p><strong>Company:</strong> {selectedInternship.company}</p>
                  <p><strong>Industry:</strong> {
                    companies.find(c => c.name === selectedInternship.company)?.industry || 'Other'
                  }</p>
                  <p><strong>Duration:</strong> {selectedInternship.duration} ({selectedInternship.startDate} to {selectedInternship.endDate})</p>
                  <p><strong>Location:</strong> {selectedInternship.location}</p>
                  <p><strong>Compensation:</strong> {selectedInternship.paid ? 
                    `${selectedInternship.stipend} (Paid)` : 'Unpaid (Academic Credit Available)'}</p>
      ...
      case 'students':
        const filteredStudents = students.filter(student =>
          ...
          (selectedInternshipStatus === 'All' || student.internshipStatus === selectedInternshipStatus)
        );
        
        return (
          <div style={styles.pageContainer}>
            <h1 style={styles.pageTitle}><FaUserGraduate style={styles.titleIcon} /> Students</h1>
              ...
              {filteredStudents.length > 0 ? (
                filteredStudents.map((student) => (
                  ...
                  >
                    ...
                    <p><strong>Internship Status:</strong> 
                      <span style={{
                        color: student.internshipStatus === 'Accepted' ? '#4CAF50' : 
                               student.internshipStatus === 'Pending' ? '#FFA500' : 
                               student.internshipStatus === 'Rejected' ? '#f44336' : '#e55b13',
                        fontWeight: 'bold'
                      }}>
                ...
                <div style={styles.studentModalContent}>
                  ...
                  <div style={styles.studentModalBody}>
                    ...
                    <div style={styles.studentInfoSection}>
                      ...
                      <p><strong>Status:</strong> 
                        <span style={{
                          color: selectedStudent.internshipStatus === 'Accepted' ? '#4CAF50' : 
                                 selectedStudent.internshipStatus === 'Pending' ? '#FFA500' : 
                                 selectedStudent.internshipStatus === 'Rejected' ? '#f44336' : '#e55b13',
                          fontWeight: 'bold'
                        }}>
                      ...
                      {selectedStudent.internshipStatus === 'Accepted' && (
                        <>
                          <p><strong>Company:</strong> {selectedStudent.internshipCompany}</p>
                          <p><strong>Supervisor:</strong> {selectedStudent.internshipSupervisor}</p>
                          <p><strong>Period:</strong> {selectedStudent.internshipStartDate} to {selectedStudent.internshipEndDate}</p>
                        </>
                      )}
                    </div>
      ...
      case 'workshops':
        return (
          <div style={styles.pageContainer}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h1 style={styles.pageTitle}><FaChalkboardTeacher style={styles.titleIcon} /> Workshops</h1>
              <button 
                onClick={() => {
                  setEditingWorkshop(null);
                  setIsWorkshopModalOpen(true);
            ...
            <div style={styles.workshopTabs}>
              <button
                style={{
                  ...
                  backgroundColor: activeWorkshopTab === 'available' ? '#05445E' : '#189AB4',
                }}
              ...
              <button
                style={{
                  ...styles.workshopTabButton,
                  backgroundColor: activeWorkshopTab === 'upcoming' ? '#05445E' : '#189AB4',
                }}
            ...
            <div style={styles.sectionContainer}>
              {workshops.filter(w => w.status === activeWorkshopTab).length > 0 ? (
                ...
                <div style={styles.modalContent}>
                  ...
                  <div style={styles.modalBody}>
                    ...
                    <div style={{ marginBottom: '15px' }}>
                      ...
                      <input
                        type="text"
                        value={editingWorkshop ? editingWorkshop.name : newWorkshop.name}
                        onChange={(e) => editingWorkshop 
                          ? setEditingWorkshop({...editingWorkshop, name: e.target.value})
                    ...
                    <div style={{ marginBottom: '15px' }}>
                      ...
                      <textarea
                        value={editingWorkshop ? editingWorkshop.description : newWorkshop.description}
                        onChange={(e) => editingWorkshop 
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px', marginBottom: '15px' }}>
                      <div>
                        ...
                        <input
                          type="date"
                          value={editingWorkshop ? editingWorkshop.startDate : newWorkshop.startDate}
                          onChange={(e) => editingWorkshop 
                            ? setEditingWorkshop({...editingWorkshop, startDate: e.target.value})
                            : setNewWorkshop({...newWorkshop, startDate: e.target.value})}
                          style={styles.input}
                        />
                      </div>
                      <div>
                        ...
                        <input
                          type="date"
                          value={editingWorkshop ? editingWorkshop.endDate : newWorkshop.endDate}
                          onChange={(e) => editingWorkshop 
                            ? setEditingWorkshop({...editingWorkshop, endDate: e.target.value})
                            : setNewWorkshop({...newWorkshop, endDate: e.target.value})}
                          style={styles.input}
                        />
                      </div>
                    </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px', marginBottom: '15px' }}>
                      <div>
                        ...
                        <input
                          type="time"
                          value={editingWorkshop ? editingWorkshop.startTime : newWorkshop.startTime}
                          onChange={(e) => editingWorkshop 
                            ? setEditingWorkshop({...editingWorkshop, startTime: e.target.value})
                      ...
                      <div>
                        ...
                        <input
                          type="time"
                          value={editingWorkshop ? editingWorkshop.endTime : newWorkshop.endTime}
                          onChange={(e) => editingWorkshop 
                            ? setEditingWorkshop({...editingWorkshop, endTime: e.target.value})
                    ...
                    <div style={{ marginBottom: '15px' }}>
                      ...
                      <input
                        type="text"
                        value={editingWorkshop ? editingWorkshop.speaker : newWorkshop.speaker}
                        onChange={(e) => editingWorkshop 
                          ? setEditingWorkshop({...editingWorkshop, speaker: e.target.value})
                          : setNewWorkshop({...newWorkshop, speaker: e.target.value})}
                        style={styles.input}
                      />
                    </div>
                    ...
                    <div style={{ marginBottom: '15px' }}>
                      ...
                      <textarea
                        value={editingWorkshop ? editingWorkshop.speakerBio : newWorkshop.speakerBio}
                        onChange={(e) => editingWorkshop 
                          ? setEditingWorkshop({...editingWorkshop, speakerBio: e.target.value})
                          : setNewWorkshop({...newWorkshop, speakerBio: e.target.value})}
                        style={styles.textarea}
                      />
                    </div>
                    ...
                    <div style={{ marginBottom: '15px' }}>
                      ...
                      <textarea
                        value={editingWorkshop ? editingWorkshop.agenda : newWorkshop.agenda}
                        onChange={(e) => editingWorkshop 
                          ? setEditingWorkshop({...editingWorkshop, agenda: e.target.value})
                          : setNewWorkshop({...newWorkshop, agenda: e.target.value})}
        ...
        case 'reports':
  const statusOptions = ['All', 'Pending', 'Flagged', 'Rejected', 'Accepted'];
  const majorOptions = ['All', ...new Set(submittedReports.map(report => report.major))];
  const filteredReports = submittedReports.filter(report =>
    (reportStatusFilter === 'All' || report.status === reportStatusFilter) &&
    (reportMajorFilter === 'All' || report.major === reportMajorFilter) &&
    (selectedReportType === 'all' || report.type === selectedReportType)
  );

        return (
          <div style={styles.pageContainer}>
            <h1 style={styles.pageTitle}><FaFileAlt style={styles.titleIcon} /> Submitted Reports</h1>
            ...
            <div style={styles.reportTypeButtons}>
              <button
                ...
                style={{
                  ...
                  backgroundColor: selectedReportType === 'internship' ? '#05445E' : '#189AB4',
                }}
              >
                Internship Reports
              </button>
              <button
                ...
                style={{
                  ...styles.reportTypeButton,
                  backgroundColor: selectedReportType === 'evaluation' ? '#05445E' : '#189AB4',
                }}
              >
                Evaluation Reports
              </button>
                ...
                <ul style={{ listStyleType: 'none', padding: 0 }}>
                  {filteredReports.map(report => (
                    ...
                    >
                      ...
                      <p style={{ margin: 0 }}><strong>Student:</strong> {report.studentName}</p>
                      <p style={{ margin: 0 }}><strong>Major:</strong> {report.major}</p>
                      <p style={{
                        margin: 0,
                        color:
                          report.status === 'Accepted' ? '#4CAF50' :
                          report.status === 'Rejected' ? '#f44336' :
                          report.status === 'Flagged' ? '#e55b13' : '#FFA500',
                        fontWeight: 'bold'
                      }}>
                        {report.status}
                      </p>
          ...
          <div style={styles.dashboardContent}>
            <div style={styles.welcomeBox}>
              ...
              <p style={styles.welcomeText}>
                Welcome to the SCAD Dashboard – your central hub for managing internships, companies, and student progress.
              </p>
            </div>
              ...
              {statsLoading ? (
                ...
                <>
                  ...
                  <div style={{ marginBottom: '30px' }}>
                    <h3 style={{ color: '#05445E', marginBottom: '15px' }}>Reports Overview</h3>
                    <div style={{ 
                      display: 'flex', 
                      gap: '20px', 
                      flexWrap: 'wrap',
                  ...
                  {/* Courses Statistics */}
                  <div style={{ marginBottom: '30px' }}>
                    <h3 style={{ color: '#05445E', marginBottom: '15px' }}>Courses in Internships</h3>
                    <div style={{ 
                      display: 'flex', 
                      gap: '20px',
                      flexWrap: 'wrap'
                      ...
                      <div style={{ 
                        ...
                        <ul style={{ paddingLeft: '20px' }}>
                          {realTimeStats.courses?.mostFrequent?.map((course, index) => (
                            <li key={index} style={{ marginBottom: '8px', display: 'flex', justifyContent: 'space-between' }}>
                              <span>{course}</span>
                              <span style={{ fontWeight: 'bold' }}>
                                {realTimeStats.courses.distribution[course]} students
                              </span>
                            </li>
                          )) || <li>No course data available</li>}
                        </ul>
                        <p style={{ marginTop: '15px', textAlign: 'right', fontSize: '14px' }}>
                          Total courses: {realTimeStats.courses?.totalCourses || 0}
                        </p>
                      </div>
                      <div style={{ 
                        flex: 3, 
                        ...
                        <div style={{ 
                          ...
                          {realTimeStats.courses?.distribution ? (
                            Object.entries(realTimeStats.courses.distribution).map(([course, count]) => {
                              const maxValue = Math.max(...Object.values(realTimeStats.courses.distribution));
                              return (
                            ...
                            <div style={{ width: '100%', textAlign: 'center' }}>No data available</div>
                          )}
                        </div>
                      </div>
                    </div>
                  ...
                  <div style={{ marginBottom: '30px' }}>
                    <h3 style={{ color: '#05445E', marginBottom: '15px' }}>Company Performance</h3>
                    <div style={{ 
                      display: 'flex', 
                      gap: '20px',
                      flexWrap: 'wrap'
                      ...
                      <div style={{ 
                        ...
                        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                          <thead>
                            <tr style={{ borderBottom: '1px solid #eee' }}>
                              <th style={{ textAlign: 'left', padding: '8px 0', fontSize: '14px' }}>Company</th>
                              <th style={{ textAlign: 'right', padding: '8px 0', fontSize: '14px' }}>Rating</th>
                            </tr>
                          </thead>
                          <tbody>
                            {realTimeStats.companies?.topRated?.map((company, index) => (
                              <tr key={index} style={{ borderBottom: '1px solid #eee' }}>
                                <td style={{ padding: '12px 0', fontSize: '14px' }}>{company.name}</td>
                        ...
                        <p style={{ marginTop: '15px', textAlign: 'right', fontSize: '14px' }}>
                          Total companies: {realTimeStats.companies?.totalCompanies || 0}
                        </p>
                      </div>
                      <div style={{ 
                        flex: 1, 
                        ...
                        <h4 style={{ marginTop: 0 }}>Top Companies by Internship Count</h4>
                        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                          <thead>
                            <tr style={{ borderBottom: '1px solid #eee' }}>
                              <th style={{ textAlign: 'left', padding: '8px 0', fontSize: '14px' }}>Company</th>
                              <th style={{ textAlign: 'right', padding: '8px 0', fontSize: '14px' }}>Internships</th>
                            </tr>
                          </thead>
                          <tbody>
                            {realTimeStats.companies?.topByCount?.map((company, index) => {
                              const maxCount = Math.max(...realTimeStats.companies.topByCount.map(c => c.count));
                              return (
                                <tr key={index} style={{ borderBottom: '1px solid #eee' }}>
                                  <td style={{ padding: '12px 0', fontSize: '14px' }}>{company.name}</td>
                                  <td style={{ textAlign: 'right', padding: '12px 0' }}>
...
<div style={{ 
  ...
  <p>Generate comprehensive reports based on current statistics and data.</p>
  ...
  <div style={{ display: 'flex', gap: '15px', marginTop: '20px' }}>
    ...
    {isGeneratingReport && (
      <div style={{ display: 'flex', alignItems: 'center', color: '#666' }}>
        <FaSync style={{ marginRight: '8px', animation: 'spin 1s linear infinite' }} />
        Generating report...
      </div>
    )}
  </div>
                  ...
                  <div style={{ marginBottom: '30px' }}>
                    <h3 style={{ color: '#05445E', marginBottom: '15px' }}>Internship Cycle</h3>
                    <div style={{ 
                      display: 'flex', 
                      gap: '20px',
                      flexWrap: 'wrap'
                      ...
                      <div style={{ 
                        ...
                        {realTimeStats.cycles?.currentCycle ? (
                          <>
                            <p><strong>Start Date:</strong> {realTimeStats.cycles.currentCycle.startDate}</p>
                            <p><strong>End Date:</strong> {realTimeStats.cycles.currentCycle.endDate}</p>
                            <p><strong>Students:</strong> {realTimeStats.cycles.studentsInCycle}</p>
                            <p><strong>Days Remaining:</strong> {realTimeStats.cycles.daysRemaining}</p>
                            <div style={{ marginTop: '15px' }}>
                              ...
                              <p style={{ textAlign: 'center', marginTop: '5px' }}>
                                {realTimeStats.cycles.completionPercentage}% Complete
                              </p>
                            </div>
                          </>
                        ) : (
            ...
            <div style={styles.cycleManagementBox}>
              ...
              {internshipCycle.isAnnounced && (
                <div style={styles.currentCycle}>
                  ...
                  <p><strong>Start Date:</strong> {internshipCycle.startDate}</p>
                  <p><strong>End Date:</strong> {internshipCycle.endDate}</p>
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={{ ...styles.sidebar, width: sidebarOpen ? '250px' : '0px' }}>
        <div style={styles.sidebarContent}>
          ...
          <ul style={styles.sidebarMenu}>
            <li 
              style={activeTab === 'dashboard' ? styles.activeMenuItem : styles.menuItem}
              onClick={() => setActiveTab('dashboard')}
            >
              <FaHome style={styles.menuIcon} /> Home
            </li>
            <li 
              style={activeTab === 'companies' ? styles.activeMenuItem : styles.menuItem}
              onClick={() => setActiveTab('companies')}
            >
              <FaBuilding style={styles.menuIcon} /> Companies
            </li>
            <li 
              style={activeTab === 'internships' ? styles.activeMenuItem : styles.menuItem}
              onClick={() => setActiveTab('internships')}
            >
              <FaBriefcase style={styles.menuIcon} /> Internships
            </li>
            <li 
              style={activeTab === 'students' ? styles.activeMenuItem : styles.menuItem}
              onClick={() => setActiveTab('students')}
            >
              <FaUserGraduate style={styles.menuIcon} /> Students
            </li>
            <li 
              style={activeTab === 'workshops' ? styles.activeMenuItem : styles.menuItem}
              onClick={() => setActiveTab('workshops')}
            >
              <FaChalkboardTeacher style={styles.menuIcon} /> Workshops
            </li>
            <li 
              style={activeTab === 'reports' ? styles.activeMenuItem : styles.menuItem}
              onClick={() => setActiveTab('reports')}
            >
              <FaFileAlt style={styles.menuIcon} /> Reports
            </li>
          </ul>
          <button style={styles.logoutButton} onClick={() => navigate('/appointments')}>
      ...
      <div style={styles.mainContentBox}>
        <div style={{ ...styles.mainContent, marginLeft: sidebarOpen ? '250px' : '0px' }}>
          {renderPageContent()}
        </div>
      </div>
